...
router.get('/user/:userId', async (request, response) => {
    try {
        const activities = await Activity.find({ userId: request.params.userId }).populate('userId', 'username email');
        response.json(activities);
    }
    catch (error) {
...
router.put('/:id', async (request, response) => {
    try {
        ...
        const activity = await Activity.findByIdAndUpdate(request.params.id, { type, duration, distance, calories, intensity, date, notes }, { new: true }).populate('userId', 'username email');
        if (!activity) {
            response.status(404).json({ error: 'Activity not found' });
            return;
        }
